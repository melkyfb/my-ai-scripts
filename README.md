...
Download CompTIA CertMaster course modules as PDFs (and videos), organized in a mirrored directory hierarchy.
...
| Argument | Description |
|---|---|
| `course_url` | Course page URL on certmaster.comptia.org |
| `-u, --user` | Login email |
| `-p, --password` | Password (prompted securely if omitted) |
| `-o, --output` | Output directory (default: `comptia-course`) |
| `--headless` | Run without showing the browser window |
| `--delay SECS` | Seconds between page loads (default: `2.0`) |
| `--skip-confirm` | Skip the outline preview confirmation |

**What it does:**

1. Opens the course URL in Chromium (visible by default — allows manual SSO/2FA handling)
2. Logs in if redirected to the login page
3. Finds and clicks the **Outline** tab
4. Discovers all numbered modules (1.0, 1.1, 1.1.1…) and shows a preview before starting
5. For each module, saves it as a PDF:

```
comptia-course/
└── 1.0 Summarize Fundamental Security Concepts/
    └── 1.1 Security Concepts/
        ├── 1.1.1 Information Security.pdf
        └── 1.1.2 Information Security Technologies.pdf
```

6. Detects embedded videos (HTML5, YouTube, Vimeo, Kaltura, Brightcove) and downloads them alongside the PDF using yt-dlp — same filename, video file extension
7. Skips already-downloaded files — safe to resume after interruption

**Examples:**

```bash
# Download with visible browser (recommended for first run / SSO)
python comptia-download.py https://certmaster.comptia.org/courses/123 -u me@email.com

# Specify output folder and password inline
python comptia-download.py <url> -u me@email.com -p secret -o ~/Downloads/sec-plus

# Fully headless (for subsequent runs after login is cached)
